...
namespace SAP_MIMOSAapp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            // Use the named HTTP client configured in Program.cs
            _httpClient = httpClientFactory.CreateClient("FastAPIClient");
            _logger = logger;
            _logger.LogInformation($"Using API base address: {_httpClient.BaseAddress}");
        }
        ...
        [HttpGet]
        public async Task<IActionResult> SystemMessage(bool improveMappings)
        {
            var response = await _httpClient.GetAsync($"system_message?improveMappings={improveMappings}");
            if (!response.IsSuccessStatusCode)
                return StatusCode((int)response.StatusCode, "Error fetching system message");
